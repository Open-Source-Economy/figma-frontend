...
import { Zap, Sparkles, CheckCircle2, DollarSign, Heart, GitBranch, Users } from 'lucide-react';
...
export const ServiceProviderCard: React.FC<ServiceProviderCardProps> = ({
  ...
  return (
    <div className="relative">
      ...
      <div className={`absolute -inset-1 bg-gradient-to-r from-brand-accent/30 via-brand-warning/25 to-brand-highlight/30 rounded-3xl blur-2xl transition-opacity duration-300 ${
      ...
      >
        ...
        <div className={`relative rounded-3xl border-2 overflow-hidden transition-all duration-300 ${
          ...
          <div className="absolute top-6 right-6 z-10 flex items-center gap-3">
            {isSelected && (
              <div className="bg-brand-accent text-white px-3 py-1 rounded-full text-xs animate-in fade-in slide-in-from-right-2 duration-300">
                ✓ Selected
              </div>
            )}
            <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all ${
              isSelected
                ? 'border-brand-accent bg-brand-accent shadow-lg shadow-brand-accent/50'
          ...
          <div className={`absolute -top-0 left-0 right-0 h-1 bg-gradient-to-r from-brand-accent via-brand-warning to-brand-highlight transition-opacity duration-300 ${
