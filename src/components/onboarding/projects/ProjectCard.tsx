import React from 'react';
import { Project } from '../../../types/DeveloperOnboarding';
import { Github, Globe, ExternalLink } from 'lucide-react';
import { ProjectRowActions } from './ProjectRowActions';
import { getProjectName, getRoleLabel, getAccessLabel, getAccessLevelClassName } from '../../../utils/projectHelpers';
import { Chip } from '../../ui/chip';

interface ProjectCardProps {
  project: Project;
  onEdit: (project: Project) => void;
  onDelete: (id: string) => void;
}

export const ProjectCard: React.FC<ProjectCardProps> = ({
  project,
  onEdit,
  onDelete,
}) => {
  return (
    <div className="rounded-xl border border-brand-neutral-300/30 bg-brand-card-blue/20 p-4 space-y-3">
      {/* Project Name & Link */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            {project.url.includes('github.com') ? (
              <Github className="w-4 h-4 text-brand-neutral-600 flex-shrink-0" />
            ) : (
              <Globe className="w-4 h-4 text-brand-neutral-600 flex-shrink-0" />
            )}
            <a
              href={project.url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-brand-neutral-800 hover:text-brand-accent transition-colors group flex items-center gap-2 min-w-0"
              title={project.url}
            >
              <span className="truncate font-medium">
                {getProjectName(project.url)}
              </span>
              <ExternalLink className="w-3.5 h-3.5 text-brand-neutral-500 group-hover:text-brand-accent flex-shrink-0 transition-colors" />
            </a>
          </div>
          <p className="text-xs text-brand-neutral-600 truncate">
            {project.url}
          </p>
        </div>
        
        {/* Action Buttons */}
        <div className="flex-shrink-0">
          <ProjectRowActions
            project={project}
            onEdit={onEdit}
            onDelete={onDelete}
          />
        </div>
      </div>

      {/* Details Grid */}
      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-brand-neutral-300/20">
        {/* Role */}
        <div>
          <p className="text-xs text-brand-neutral-600 uppercase tracking-wider mb-1">
            Your Role
          </p>
          <p className="text-sm text-brand-neutral-700">
            {getRoleLabel(project.role)}
          </p>
        </div>

        {/* Access Level */}
        <div>
          <p className="text-xs text-brand-neutral-600 uppercase tracking-wider mb-1">
            Access Level
          </p>
          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs ${getAccessLevelClassName(project.mainBranchAccess)}`}>
            {getAccessLabel(project.mainBranchAccess)}
          </span>
        </div>
      </div>

      {/* Ecosystems */}
      {project.ecosystems && project.ecosystems.length > 0 && (
        <div className="pt-3 border-t border-brand-neutral-300/20">
          <p className="text-xs text-brand-neutral-600 uppercase tracking-wider mb-2">
            Ecosystems
          </p>
          <div className="flex flex-wrap gap-1.5">
            {project.ecosystems.map((ecosystem) => (
              <Chip key={ecosystem} size="sm">
                {ecosystem}
              </Chip>
            ))}
          </div>
        </div>
      )}
    </div>
  );
};
