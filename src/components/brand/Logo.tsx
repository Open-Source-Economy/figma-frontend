...
export function Logo({ size = 'md', variant = 'primary', showText = true, className = '' }: LogoProps) {
  const iconColorClass = iconVariantClasses[variant];
  const textColorClass = textVariantClasses[variant];
  
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div className={`${sizeClasses[size]} flex-shrink-0 ${iconColorClass}`}>
      ...
      {showText && (
        <div className="flex flex-col">
          <span className={`leading-tight ${textColorClass}`}>
            Open Source
          </span>
          <span className={`leading-tight ${textColorClass} opacity-80`}>
            Economy
          </span>
        </div>
      )}
