...
export function TokenDisplay({ tokens, variant = 'compact', className = '' }: TokenDisplayProps) {
  if (variant === 'badge') {
    return (
      <div className={`flex items-center gap-1.5 px-2.5 py-1.5 bg-brand-accent/10 border border-brand-accent/20 rounded-md hover:opacity-80 transition-opacity cursor-pointer ${className}`}>
