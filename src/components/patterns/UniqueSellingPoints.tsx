...
export function UniqueSellingPoints({ className = '' }: UniqueSellingPointsProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {sellingPoints.map((point, index) => (
            ...
            >
              {/* Background Glow */}
              <div className={`absolute inset-0 bg-gradient-to-br from-${point.accentColor}/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl`}></div>
              
              {/* Icon */}
              <div className={`w-12 h-12 bg-gradient-to-br from-${point.accentColor} to-${point.accentColor}-dark rounded-xl flex items-center justify-center mx-auto mb-6 shadow-lg group-hover:scale-110 transition-transform duration-300 relative z-10`}>
                {point.icon}
              </div>
