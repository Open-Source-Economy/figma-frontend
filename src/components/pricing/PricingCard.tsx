...
import { BillingCycle, FeatureSection } from './types';
...
export function PricingCard({
  ...
  const originalPrice = billingCycle === 'annual' ? monthlyPrice : null;
        ...
        {sections.map((section, sectionIndex) => {
          ...
          return (
            <div 
              key={sectionIndex} 
              className={`${
                isCoreBenefit ? 'flex-1' : ''
              } ${
                isOpenSourceImpact ? 'mt-6' : ''
              } ${
            ...
            >
              {isOpenSourceImpact ? (
                ...
                <RecognitionSection 
                  ...
                  subtitle={section.subtitle}
                  features={section.features}
                  tier={planId as 'individual' | 'bronze' | 'silver' | 'gold'}
                />
              )}
            </div>
          );
