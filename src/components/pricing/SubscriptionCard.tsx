    ...
    <div className={`bg-brand-secondary rounded-xl border ${getTierBorderColor(tier as any)} shadow-xl overflow-hidden`}>
      ...
      <div className="px-6 pb-6 border-t border-brand-neutral-300/20 pt-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <BenefitsSection
            title="Core Mission Benefits"
            benefits={commonBenefits}
            iconColor="accent"
          />
          
          <BenefitsSection
            title={`${getPlanName(tier as any)} Tier Benefits`}
            benefits={tierBenefits}
            iconColor={tier as any}
          />
        </div>
      </div>
      ...
      <div className="px-6 pb-6 flex items-center justify-between gap-6 text-sm border-t border-brand-neutral-300/20 pt-4">
        ...
        <Button
          onClick={onManageSubscription}
          variant="outline"
          className="bg-brand-error/10 hover:bg-brand-error/20 border-brand-error/30 hover:border-brand-error/50 text-brand-error hover:text-brand-error transition-colors"
        >
