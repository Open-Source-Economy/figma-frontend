...
export const LoadingState: React.FC<LoadingStateProps> = ({
  ...
  const SpinnerContent = () => (
    <div className={`flex flex-col items-center justify-center ${sizeClasses.container[size]} ${className}`}>
      ...
      {message && (
        <p className={`${sizeClasses.text[size]} text-brand-neutral-600 text-center`}>
          {message}
        </p>
      )}
    </div>
  ...
  const DotsContent = () => (
    <div className={`flex items-center justify-center ${sizeClasses.container[size]} ${className}`}>
      <div className="flex items-center gap-2">
        <div className={`${size === 'sm' ? 'w-2 h-2' : size === 'lg' ? 'w-4 h-4' : 'w-3 h-3'} rounded-full bg-brand-accent animate-bounce`} style={{ animationDelay: '0ms' }} />
        <div className={`${size === 'sm' ? 'w-2 h-2' : size === 'lg' ? 'w-4 h-4' : 'w-3 h-3'} rounded-full bg-brand-accent animate-bounce`} style={{ animationDelay: '150ms' }} />
        <div className={`${size === 'sm' ? 'w-2 h-2' : size === 'lg' ? 'w-4 h-4' : 'w-3 h-3'} rounded-full bg-brand-accent animate-bounce`} style={{ animationDelay: '300ms' }} />
      </div>
      {message && (
        <p className={`ml-3 ${sizeClasses.text[size]} text-brand-neutral-600`}>
          {message}
        </p>
      )}
    </div>
  ...
  const PulseContent = () => (
    <div className={`flex flex-col items-center justify-center ${sizeClasses.container[size]} ${className}`}>
      ...
      {message && (
        <p className={`mt-3 ${sizeClasses.text[size]} text-brand-neutral-600 text-center`}>
          {message}
        </p>
      )}
    </div>
  ...
  const InlineContent = () => (
    <span className={`inline-flex items-center gap-2 ${className}`}>
      <Loader2 className={`${sizeClasses.spinner[size]} text-brand-accent animate-spin`} />
      {message && (
        <span className={`${sizeClasses.text[size]} text-brand-neutral-600`}>
          {message}
        </span>
      )}
    </span>
...
export const LoadingCard: React.FC<{
  ...
  <div className={`bg-brand-card-blue border border-brand-neutral-300/30 rounded-xl p-6 ${className}`}>
    <LoadingState
      variant="skeleton"
      size="md"
      skeletonConfig={{ lines, showAvatar, showImage }}
    />
  </div>
);
