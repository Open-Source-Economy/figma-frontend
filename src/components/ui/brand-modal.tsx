 ...
 * 
 * Features:
 * - Premium dark blue styling matching brand card system
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      ...
      >
        ...
        <DialogHeader className={cn('space-y-3 px-6 pt-6 pb-4 flex-shrink-0', headerClassName)}>
          <DialogTitle className="text-brand-neutral-900">
            {title}
          </DialogTitle>
          {description && (
        ...
        {footer && (
          <DialogFooter className={cn(
            'px-6 pt-4 pb-6 border-t border-brand-neutral-300/10 flex-shrink-0',
            footerClassName
          )}>
            {footer}
          </DialogFooter>
...
export const BrandModalSection: React.FC<BrandModalSectionProps> = ({
  icon,
  title,
  description,
  iconColor = 'accent',
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      {/* Section Content */}
      <div className="bg-brand-card-blue/30 rounded-xl border border-brand-neutral-300/10 p-5">
        {children}
      </div>
    </div>
  );
...
export const BrandModalAlert: React.FC<BrandModalAlertProps> = ({
  type,
  icon,
  title,
  children,
    ...
    <div className={cn(
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="space-y-2">
          ...
          <div className={cn('text-sm leading-relaxed', classes.text)}>
            {children}
          </div>
        </div>
      </div>
